...
<body class="text-white bg-hero min-h-screen">
        ...
        try {
            ...
            if (!response.ok) {
                Swal.fire({
                    icon: 'error',
                    title: 'Save failed!',
                    text: (data && (data.message || data.error)) ? (data.message || data.error) : 'Please try again.'
                });
                return;
            }
    ...
    async function buildFinalCanvas() {
        ...
        if (logoEl && logoEl.src) {
            ...
            const pad = size * 0.12;
            const bgX = x - pad, bgY = y - pad, bgW = size + pad * 2, bgH = size + pad * 2;
    ...
    downloadImgBtn.addEventListener('click', async () => {
        if (!qrCode) {
            Swal.fire({ icon: 'info', title: 'No QR Found!', text: 'Please generate first.' });
            return;
        }

        const finalCanvas = await buildFinalCanvas();
        if (!finalCanvas) {
            Swal.fire({ icon: 'error', title: 'QR not ready', text: 'Please generate again.' });
            return;
        }
